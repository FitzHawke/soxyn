...
const scale = (size: number) => (settings.overview.scale / 100) * size;
...
export default (id: number) => {
  ...
  return Widget.Box({
    ...
    vpack: "center",
    css: `
            min-width: ${(settings.overview.scale / 100) * size(id).w}px;
            min-height: ${(settings.overview.scale / 100) * size(id).h}px;
        `,
    setup(box) {
